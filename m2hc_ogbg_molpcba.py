...
class Model(nn.Module):
    def __init__(self,emb_dim=290,hidden1_dim=100,hidden2_dim=100,random_walk_dim=20,k_hop_adj=None,random_walk_feats=None,num_classes=6,dropout=0.,num_layer_mhmc = 1):
        super().__init__()
...
class Subgraph_GNN(nn.Module):
    def __init__(self,in_dim, hidden1_dim,hidden2_dim, k_hop_adj=[0,0,0,0],random_walk_feats=None,dropout = 0.,num_layers=2):
        super().__init__()
        ...
        for i in range(num_layers):
            if i==0:
                layer0 = nn.Sequential(nn.Linear(in_dim,hidden1_dim),nn.BatchNorm1d(hidden1_dim),nn.ReLU(),nn.Dropout(dropout),nn.Linear(hidden1_dim,hidden2_dim),nn.BatchNorm1d(hidden1_dim))
                layer1 = nn.Sequential(nn.Linear(in_dim, hidden1_dim),nn.BatchNorm1d(hidden1_dim), nn.ReLU(),nn.Dropout(dropout),nn.Linear(hidden1_dim, hidden2_dim),nn.BatchNorm1d(hidden1_dim))
                layer2 = nn.Sequential(nn.Linear(in_dim, hidden1_dim),nn.BatchNorm1d(hidden1_dim), nn.ReLU(),nn.Dropout(dropout),nn.Linear(hidden1_dim, hidden2_dim),nn.BatchNorm1d(hidden1_dim))
                layer3 = nn.Sequential(nn.Linear(in_dim, hidden1_dim),nn.BatchNorm1d(hidden1_dim), nn.ReLU(),nn.Dropout(dropout),nn.Linear(hidden1_dim, hidden2_dim),nn.BatchNorm1d(hidden1_dim))
                self.model_dict[str(i)]= nn.ModuleList([layer0,layer1,layer2,layer3])
            else:
                layer0 = nn.Sequential(nn.Linear(hidden2_dim,hidden1_dim),nn.BatchNorm1d(hidden1_dim),nn.ReLU(),nn.Dropout(dropout),nn.Linear(hidden1_dim,hidden2_dim),nn.BatchNorm1d(hidden1_dim))
                layer1 = nn.Sequential(nn.Linear(hidden2_dim, hidden1_dim),nn.BatchNorm1d(hidden1_dim), nn.ReLU(),nn.Dropout(dropout),nn.Linear(hidden1_dim, hidden2_dim),nn.BatchNorm1d(hidden1_dim))
                layer2 = nn.Sequential(nn.Linear(hidden2_dim, hidden1_dim),nn.BatchNorm1d(hidden1_dim), nn.ReLU(),nn.Dropout(dropout),nn.Linear(hidden1_dim, hidden2_dim),nn.BatchNorm1d(hidden1_dim))
                layer3 = nn.Sequential(nn.Linear(hidden2_dim, hidden1_dim),nn.BatchNorm1d(hidden1_dim), nn.ReLU(),nn.Dropout(dropout),nn.Linear(hidden1_dim, hidden2_dim),nn.BatchNorm1d(hidden1_dim))
                self.model_dict[str(i)]= nn.ModuleList([layer0,layer1,layer2,layer3])
...
class PL_GCN(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        val_ap = evaluator.eval(input_dict)['ap']
        input_dict = {"y_true": y_true_test, "y_pred": y_pred_test}
        test_ap = evaluator.eval(input_dict)['ap']
        exp_data.append([val_ap,test_ap])
        res = {'val_ap':val_ap,'test_ap':test_ap}
        self.log_dict(res)
    ...
    def validation_step(self, batch, batch_idx,dataloader_idx):
        ...
        if batch.x.shape[0]==1:
            ...
            if not self.benchmark:
                ...
                # val = evaluator.eval(input_dict)['ap']
...
if __name__=='__main__':
    ...
    dataset = OGB_Dataset(root='data/ogb/'+'molpcba')
    train_loader = DataLoader(dataset[split_idx["train"]], batch_size=64, shuffle=True)
    valid_loader = DataLoader(dataset[split_idx["valid"]], batch_size=80, shuffle=False)
    test_loader = DataLoader(dataset[split_idx["test"]], batch_size=80, shuffle=False)
    for j in range(5):
        ...
        for b in [False]:
            for r in [walk_order]:
                ...
                pl_model = PL_GCN(emb_dim=290,num_classes=num_class, use_benchamark=b, random_walk_dim=r,node_classification=False,lr=1e-3,dropout=dropout_rate,hidden1_dim=hidden_dims,hidden2_dim=hidden_dims,num_layer_mhmc=num_layers_mhmc)
                trainer = pl.Trainer(auto_select_gpus=True,default_root_dir=f'saved_models/ogb_molpcba/gin_mhmc_0816/',gpus=gpu_num, max_epochs=150)
                ...
                print (f'iter_num:{j},dropout:{dropout_rate},random_order:{r},result for top1:{top1},top3:{top3}')
