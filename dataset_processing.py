...
from func_util import collate_graph_adj
...
class Processed_Dataset(InMemoryDataset):
    ...
    def process(self):
        ...
        name = 'ENZYMES'   # to process another dataset, change the name here
        tu_dset = TUDataset(root='data/TUDataset/', name=name)
        print ('total len:',len(tu_dset))
        data_list = []
        cnt =0
        for t in tu_dset:
            if cnt%50==0:
        ...
        data_list = [i for i in data_list if i.x.shape[0]==i.rand_feature.shape[0]]
...
if __name__=='__main__':
    ...
    #     if max(d.y)>1:
    #         print (d.y)
    Processed_Dataset(root='data/TUDataset/'+'enzymes_new')
